...
class TimesheetPeriodService
{
    ...
    public function getDaysOfTheTimesheetPeriod($xml, string $currentDate)
    {
        ...
        if ($className === 'WeeklyTimesheetPeriod') {
            $className = 'MonthlyTimesheetPeriod';
            $xml->StartDate = self::DEFAULT_TIMESHEET_START_DATE;
        }
        $timesheetPeriodObject = $timesheetPeriodFactory->createTimesheetPeriod($className);
    ...
    public function getTimesheetHeading(): string
    {
        $xmlString = $this->getConfigService()->getTimeSheetPeriodConfig();
        $xml = simplexml_load_string($xmlString);
        if ((string)$xml->ClassName === 'WeeklyTimesheetPeriod') {
            return 'Month';
        }
        return $xml->Heading;
    }
    ...
    public function getTimesheetStartDate(): string
    {
        ...
        $xml = simplexml_load_string($xmlString);
        if ((string)$xml->ClassName === 'WeeklyTimesheetPeriod') {
            return (string)self::DEFAULT_TIMESHEET_START_DATE;
        }

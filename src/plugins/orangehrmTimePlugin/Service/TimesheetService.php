...
class TimesheetService
{
    ...
    {
        $periodDates = $this->getTimesheetPeriodService()
            ->getDefinedTimesheetPeriod($this->getDateTimeHelper()->formatDateTimeToYmd($date));

        if (empty($periodDates)) {
            return $this->getDateTimeHelper()->getWeekBoundaryForGivenDate($date, '1');
        }

        $startDate = substr((string)reset($periodDates), 0, 10);
        $endDate = substr((string)end($periodDates), 0, 10);

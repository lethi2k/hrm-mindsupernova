...
class TimesheetDao extends BaseDao
{
    ...
    public function getEmployeeTimesheetPaginator(
        ...
        $q->leftJoin(TimesheetItem::class, 'timesheetItem', 'WITH', 'timesheetItem.timesheet = timesheet');
        ...
        if (!is_null($employeeTimesheetActionSearchFilterParams->getFromDate())) {
            $q->andWhere('timesheet.endDate >= :fromDate')
                ->setParameter('fromDate', $employeeTimesheetActionSearchFilterParams->getFromDate());
        }

        if (!is_null($employeeTimesheetActionSearchFilterParams->getToDate())) {
            $q->andWhere('timesheet.startDate <= :toDate')
                ->setParameter('toDate', $employeeTimesheetActionSearchFilterParams->getToDate());
        }

        $hasLoggedTimeSubQuery = $this->createQueryBuilder(TimesheetItem::class, 'timesheetItemSub')
            ->select('1')
            ->leftJoin('timesheetItemSub.timesheet', 'timesheetSub')
            ->andWhere('timesheetSub.employee = timesheet.employee')
            ->andWhere('timesheetItemSub.duration > 0');

        if (!is_null($employeeTimesheetActionSearchFilterParams->getFromDate())) {
            $hasLoggedTimeSubQuery->andWhere('timesheetSub.endDate >= :fromDate');
        }

        if (!is_null($employeeTimesheetActionSearchFilterParams->getToDate())) {
            $hasLoggedTimeSubQuery->andWhere('timesheetSub.startDate <= :toDate');
        }
        ...
        $q->groupBy('timesheet.id');
        if (!is_null($employeeTimesheetActionSearchFilterParams->hasLoggedTime())) {
            if ($employeeTimesheetActionSearchFilterParams->hasLoggedTime()) {
                $q->having('COALESCE(SUM(timesheetItem.duration), 0) > 0');
            } else {
                $q->andWhere($q->expr()->not($q->expr()->exists($hasLoggedTimeSubQuery->getDQL())));
            }
        }

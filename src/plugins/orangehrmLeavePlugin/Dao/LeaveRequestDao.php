...
class LeaveRequestDao extends BaseDao
{
    ...
    private function getLeaveRequestsPaginator(
        ...
        $q = $this->createQueryBuilder(LeaveRequest::class, 'leaveRequest')
            ->innerJoin('leaveRequest.leaves', 'leave')
    ...
    /**
     * Hard delete leave requests and all related records.
     *
     * @param int[] $leaveRequestIds
     * @throws TransactionException
     */
    public function hardDeleteLeaveRequestsByIds(array $leaveRequestIds): void
    {
        ...
        $this->beginTransaction();
        try {
            $leaveIds = $this->createQueryBuilder(Leave::class, 'l')
                ->select('l.id')
                ->andWhere('l.leaveRequest IN (:leaveRequestIds)')
                ->setParameter('leaveRequestIds', $leaveRequestIds)
                ->getQuery()
            ...
            if (!empty($leaveIds)) {
                /** @var LeaveLeaveEntitlement[] $leaveLeaveEntitlements */
                $leaveLeaveEntitlements = $this->createQueryBuilder(LeaveLeaveEntitlement::class, 'lle')
                    ->andWhere('lle.leave IN (:leaveIds)')
                ...
                foreach ($leaveLeaveEntitlements as $leaveLeaveEntitlement) {
                    $leaveEntitlement = $leaveLeaveEntitlement->getEntitlement();
                    $updatedDaysUsed = $leaveEntitlement->getDaysUsed() - $leaveLeaveEntitlement->getLengthDays();
                    $leaveEntitlement->setDaysUsed(max(0, $updatedDaysUsed));
                    $this->persist($leaveEntitlement);
                }

                $this->createQueryBuilder(LeaveLeaveEntitlement::class, 'lle')
                    ->delete()
                    ->andWhere('lle.leave IN (:leaveIds)')
                    ->setParameter('leaveIds', $leaveIds)
                    ->getQuery()
                ...
                $this->createQueryBuilder(Leave::class, 'l')
                    ->delete()
                    ->andWhere('l.id IN (:leaveIds)')
                    ->setParameter('leaveIds', $leaveIds)
                    ->getQuery()
                    ->execute();
            }

            $this->createQueryBuilder(LeaveRequestComment::class, 'lrc')
                ->delete()
                ->andWhere('lrc.leaveRequest IN (:leaveRequestIds)')
                ->setParameter('leaveRequestIds', $leaveRequestIds)
                ->getQuery()
                ->execute();

            $this->createQueryBuilder(LeaveRequest::class, 'lr')
                ->delete()
                ->andWhere('lr.id IN (:leaveRequestIds)')
                ->setParameter('leaveRequestIds', $leaveRequestIds)
                ->getQuery()
                ->execute();

...
class EmployeeLeaveRequestAPI extends Endpoint implements CrudEndpoint
{
    ...
    public function update(): EndpointResult
    {
        ...
        if ($action === self::ACTION_DELETE) {
            $this->getLeaveRequestService()->hardDeleteLeaveRequest($leaveRequest);
            return new EndpointResourceResult(ArrayModel::class, [$leaveRequestId]);
        }
    ...
    public function delete(): EndpointResult
    {
        ...
        $leaveRequests = $this->getLeaveRequestService()->getLeaveRequestDao()->getLeaveRequestsByLeaveRequestIds($ids);
        $this->throwRecordNotFoundExceptionIfEmptyIds($leaveRequests);

        $processedLeaveRequestIds = [];
        foreach ($leaveRequests as $leaveRequest) {
            $this->checkLeaveRequestAccessible($leaveRequest);
            $detailedLeaveRequest = new DetailedLeaveRequest($leaveRequest);
            $detailedLeaveRequest->fetchLeaves();
            $this->getLeaveRequestService()->forceCancelLeaveRequest($detailedLeaveRequest);
            $processedLeaveRequestIds[] = $leaveRequest->getId();
        }

        return new EndpointResourceResult(ArrayModel::class, $processedLeaveRequestIds);
    }

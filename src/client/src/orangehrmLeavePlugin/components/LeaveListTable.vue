...
export default {
  ...
  setup(props) {
    ...
    const rules = {
      fromDate: [validDateFormat(userDateFormat)],
      toDate: [
    ...
    const serializedFilters = computed(() => {
      ...
      return {
        ...
        includeEmployees: filters.value.includePastEmps
          ? 'currentAndPast'
          : 'onlyCurrent',
        statuses: statusIds.length > 0 ? statusIds : undefined,
  ...
  beforeMount() {
    if (this.filters.statuses.length === 0) {
      this.filters.statuses = this.leaveStatuses;
    }
    if (!this.myLeaveList) {
      // On admin leave list, default to all employees and all dates.
      this.filters.includePastEmps = true;
  ...
  methods: {
    cellRenderer(...[, , , row]) {
      ...
      if (!this.myLeaveList) {
        dropdownActions.push({
          label: this.$t('performance.delete'),
          context: 'delete_leave',
        });
      }
    ...
    onLeaveDropdownAction(event, item) {
      switch (event.context) {
        ...
        case 'cancel_leave':
          ...
          break;
        case 'delete_leave':
          this.onDeleteLeaveRequest(item.id);
          break;
        case 'pim_details':
    ...
    onDeleteLeaveRequest(id) {
      this.isLoading = true;
      this.processLeaveRequestAction(id, 'DELETE')
        .then(() => {

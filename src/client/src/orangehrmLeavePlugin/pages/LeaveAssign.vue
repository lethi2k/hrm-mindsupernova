...
export default {
  ...
  methods: {
    getSanitizedLeaveForSubmit() {
      const leaveData = {
        ...this.leave,
        duration: {...this.leave.duration},
        endDuration: {...this.leave.endDuration},
      };
      if (this.appliedLeaveDuration > 1 && leaveData.partialOptions?.key) {
        const partialKey = leaveData.partialOptions.key;
        const hasStartDuration = !!leaveData.duration?.type;
        const hasEndDuration = !!leaveData.endDuration?.type;
        // If partial-day selection is incomplete, fallback to normal multi-day leave.
        if (
          (partialKey === 'all' && !hasStartDuration) ||
          (partialKey === 'start' && !hasStartDuration) ||
          (partialKey === 'end' && !hasEndDuration) ||
          (partialKey === 'start_end' && (!hasStartDuration || !hasEndDuration))
        ) {
          leaveData.partialOptions = null;
          leaveData.duration.type = null;
          leaveData.endDuration.type = null;
        }
      }
      return leaveData;
    },
    onSave() {
      ...
      const leaveData = this.getSanitizedLeaveForSubmit();

      this.http
        .create(this.serializeBody(leaveData))
        .then(() => {
